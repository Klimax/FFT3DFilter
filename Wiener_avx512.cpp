...
//-----------------------------------------------------------------------------------------
//
void WienerFilter::ApplyWiener2D_AVX512() noexcept
{
	int w(0);
	...
	if (sharpen == 0 && dehalo == 0)// no sharpen, no dehalo
	{
		for (int block = start_block; block < blocks; block++)
		{
			...
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first v.1.2
				{
					__m512 r1 = _mm512_load_ps(outcur[w]);
					__m512 r3 = _mm512_mul_ps(r1, r1);
					__m512 r2 = _mm512_permute_ps(r3, _MM_SHUFFLE(0, 3, 0, 1));
					r3 = _mm512_add_ps(r3, r2);
					r3 = _mm512_add_ps(r3, _mm512_set1_ps(1e-15f));
					r2 = _mm512_sub_ps(r3, _mm512_set1_ps(sigmaSquaredNoiseNormed));
					r3 = _mm512_div_ps(r2, r3);
					r3 = _mm512_max_ps(r3, _mm512_set1_ps(lowlimit));
					r3 = _mm512_moveldup_ps(r3);
					r1 = _mm512_mul_ps(r1, r3);
					_mm512_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
				for (; w < outwidth; w++) // not skip first v.1.2
				{
		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first
				{
					...
					r4 = _mm512_add_ps(r4, _mm512_set1_ps(1));
					r4 = _mm512_moveldup_ps(r4);
					r3 = _mm512_mul_ps(r3, r4);
					r1 = _mm512_mul_ps(r1, r3);
					_mm512_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
				_mm_prefetch((const char*)(wsharpen + outpitch), _MM_HINT_T0);
				for (; w < outwidth; w++) // not skip first
				{
					psd = (outcur[w][0] * outcur[w][0] + outcur[w][1] * outcur[w][1]) + 1e-15f;// power spectrum density
					WienerFactor = max((psd - sigmaSquaredNoiseNormed) / psd, lowlimit); // limited Wiener filter
		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first
				{
					...
					__m512 r7 = _mm512_mul_ps(r3, wdehalo8);
					__m512 r5 = _mm512_add_ps(r3, _mm512_set1_ps(ht2n));
					r7 = _mm512_mul_ps(r7, _mm512_set1_ps(dehalo));
					r7 = _mm512_add_ps(r5, r7);
					r5 = _mm512_div_ps(r5, r7);
					...
					r5 = _mm512_moveldup_ps(r5);
					r3 = _mm512_mul_ps(r3, r5);
					r1 = _mm512_mul_ps(r1, r3);
					_mm512_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first
				{
					...
					__m512 wdehalo8 = _mm512_castps256_ps512(_mm256_load_ps(&wdehalo[w]));
					wdehalo8 = _mm512_permutexvar_ps(_mm512_set_epi32(7, 7, 6, 6, 5, 5, 4, 4, 3, 3, 2, 2, 1, 1, 0, 0), wdehalo8);
					__m512 r8 = _mm512_mul_ps(r3, wdehalo8);
					__m512 r9 = _mm512_add_ps(r3, _mm512_set1_ps(ht2n));
					r8 = _mm512_mul_ps(r8, _mm512_set1_ps(dehalo));
					r8 = _mm512_add_ps(r9, r8);
					r9 = _mm512_div_ps(r9, r8);

					r4 = _mm512_mul_ps(r4, r9);
					r4 = _mm512_add_ps(r4, _mm512_set1_ps(1));
					r4 = _mm512_moveldup_ps(r4);
					r3 = _mm512_mul_ps(r3, r4);
					r1 = _mm512_mul_ps(r1, r3);
...
void WienerFilter::ApplyWiener2D_degrid_AVX512() noexcept
{
	int w(0);
	float psd(0.0f), WienerFactor(0.0f);
	const int outwidth8 = outwidth - outwidth % 8;
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first v.1.2
				{
					...
					r3 = _mm512_add_ps(r3, _mm512_set1_ps(1e-15f)); //psd
					r2 = _mm512_sub_ps(r3, _mm512_set1_ps(sigmaSquaredNoiseNormed));
					r3 = _mm512_div_ps(r2, r3);
					r3 = _mm512_max_ps(r3, _mm512_set1_ps(lowlimit)); //wienerfactor
					r3 = _mm512_moveldup_ps(r3);
					r1 = _mm512_fmadd_ps(r4, r3, gridcorrection);
					_mm512_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first
				{
					...
					__m512 r2 = _mm512_permute_ps(r3, _MM_SHUFFLE(0, 3, 0, 1));
					r3 = _mm512_add_ps(r3, r2); //psd
					__m512 psd4 = _mm512_add_ps(r3, _mm512_set1_ps(1e-15f)); //psd [0,2]
					psd4 = _mm512_moveldup_ps(psd4);
					r2 = _mm512_sub_ps(psd4, _mm512_set1_ps(sigmaSquaredNoiseNormed));
					r3 = _mm512_div_ps(r2, psd4);
					__m512 wienerfactor8 = _mm512_max_ps(r3, _mm512_set1_ps(lowlimit)); //wienerfactor

					__m512 sharp8 = _mm512_castps256_ps512(_mm256_load_ps(&wsharpen[w]));
					sharp8 = _mm512_permutexvar_ps(_mm512_set_epi32(7, 7, 6, 6, 5, 5, 4, 4, 3, 3, 2, 2, 1, 1, 0, 0), sharp8);
					__m512 r5 = _mm512_mul_ps(_mm512_set1_ps(sharpen), sharp8);
					__m512 r6 = _mm512_add_ps(psd4, _mm512_set1_ps(sigmaSquaredSharpenMax));
					__m512 r7 = _mm512_add_ps(psd4, _mm512_set1_ps(sigmaSquaredSharpenMin));
					__m512 r8 = _mm512_mul_ps(psd4, _mm512_set1_ps(sigmaSquaredSharpenMax));

					r7 = _mm512_mul_ps(r6, r7);
					r8 = _mm512_div_ps(r8, r7);
					r6 = _mm512_sqrt_ps(r8);

					r5 = _mm512_permute_ps(r5, _MM_SHUFFLE(1, 1, 0, 0));
					r5 = _mm512_fmadd_ps(r5, r6, _mm512_set1_ps(1.0f));
					wienerfactor8 = _mm512_mul_ps(r5, wienerfactor8);

					r2 = _mm512_fmadd_ps(r4, wienerfactor8, gridcorrection);
					_mm512_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
				_mm_prefetch((const char*)(gridsample + outpitch), _MM_HINT_T0);
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first
				{
					...
					__m512 r5 = _mm512_add_ps(psd4, _mm512_set1_ps(ht2n));
					__m512 r6 = _mm512_mul_ps(psd4, _mm512_set1_ps(dehalo));
					__m512 wdehalo8 = _mm512_castps256_ps512(_mm256_load_ps(&wdehalo[w]));
					wdehalo8 = _mm512_permutexvar_ps(_mm512_set_epi32(7, 7, 6, 6, 5, 5, 4, 4, 3, 3, 2, 2, 1, 1, 0, 0), wdehalo8);
					r6 = _mm512_fmadd_ps(r6, wdehalo8, r5);

					r5 = _mm512_div_ps(r5, r6);
					wienerfactor8 = _mm512_mul_ps(r5, wienerfactor8);

					r1 = _mm512_fmadd_ps(r4, wienerfactor8, gridcorrection);
					_mm512_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
				_mm_prefetch((const char*)(gridsample + outpitch), _MM_HINT_T0);
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (w = 0; w < outwidth8; w = w + 8) // not skip first
				{
					...
					r5 = _mm512_add_ps(_mm512_set1_ps(1.0f), r5);
					wienerfactor8 = _mm512_mul_ps(r5, wienerfactor8);
...
void WienerFilter::ApplyWiener3D2_AVX512() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++)
		{
			...
			{
				__m512 sum8 = _mm512_add_ps(r3, r2);
				__m512 dif8 = _mm512_sub_ps(r3, r2);
				__m512 r1 = _mm512_mul_ps(sum8, sum8);
				...
				r1 = _mm512_sub_ps(psds8, _mm512_set1_ps(sigmaSquaredNoiseNormed));
				r2 = _mm512_sub_ps(psdd8, _mm512_set1_ps(sigmaSquaredNoiseNormed));
...
void WienerFilter::ApplyWiener3D2_degrid_AVX512() noexcept
{
	// dft 3d (very short - 2 points)
	float psd(0.0f), WienerFactor(0.0f), f3d0r(0.0f), f3d1r(0.0f), f3d0i(0.0f), f3d1i(0.0f);
	int w(0);
	...
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++)
		{
			...
			for (w = 0; w < outwidth8; w = w + 8)
			{
				...
				gridcorrection = _mm512_mul_ps(gridcorrection, _mm512_set1_ps(2.0));
				__m512 sum8 = _mm512_add_ps(r3, r2);
				sum8 = _mm512_sub_ps(sum8, gridcorrection);
				__m512 dif8 = _mm512_sub_ps(r3, r2);
				__m512 r1 = _mm512_mul_ps(sum8, sum8);
				r2 = _mm512_mul_ps(dif8, dif8);
				...
				r2 = _mm512_load_ps(outprev[w + 8]);
				r3 = _mm512_load_ps(outcur[w + 8]);
				r1 = _mm512_add_ps(gridcorrection, r1);
				r1 = _mm512_mul_ps(r1, _mm512_set1_ps(0.5f));
				_mm512_store_ps(outprev[w], r1);
			}
			_mm_prefetch((const char*)(outprev + outpitch), _MM_HINT_T0);
...
void WienerFilter::ApplyWiener3D3_AVX512() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (w = 0; w < outwidth8; w = w + 8) // 
			{
				...
				d8 = _mm512_mask_sub_ps(d8, 0X55, r2, r1); //r,i,r,i!
				...
				r1 = _mm512_shuffle_ps(psdc8, psdc8, _MM_SHUFFLE(0, 3, 0, 1));//psd,-,psd,-
				r2 = _mm512_shuffle_ps(psdp8, psdp8, _MM_SHUFFLE(0, 3, 0, 1));
				r3 = _mm512_shuffle_ps(psdn8, psdn8, _MM_SHUFFLE(0, 3, 0, 1));
				...
				r1 = _mm512_sub_ps(psdc8, _mm512_set1_ps(sigmaSquaredNoiseNormed));
				r2 = _mm512_sub_ps(psdp8, _mm512_set1_ps(sigmaSquaredNoiseNormed));
				r3 = _mm512_sub_ps(psdn8, _mm512_set1_ps(sigmaSquaredNoiseNormed));
				...
				fc8 = _mm512_mul_ps(_mm512_moveldup_ps(r1), fc8);
				fp8 = _mm512_mul_ps(_mm512_moveldup_ps(r2), fp8);
				fn8 = _mm512_mul_ps(_mm512_moveldup_ps(r3), fn8);
...
void WienerFilter::ApplyWiener3D3_degrid_AVX512() noexcept
{
	// dft 3d (very short - 3 points)
	float fcr(0.0f), fci(0.0f), fpr(0.0f), fpi(0.0f), fnr(0.0f), fni(0.0f);
	float WienerFactor(0.0f), psd(0.0f);
	...
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (w = 0; w < outwidth8; w = w + 8) // 
			{
				...
				__m512 d8 = _mm512_sub_ps(r1, r2); //r,i,r,i!
				d8 = _mm512_mask_sub_ps(d8, 0XFF, r2, r1); //r,i,r,i!
				d8 = _mm512_mul_ps(d8, _mm512_set1_ps(sin120));
				d8 = _mm512_permute_ps(d8, _MM_SHUFFLE(2, 3, 0, 1));
				r1 = _mm512_fmadd_ps(pn8, _mm512_set1_ps(-0.5f), r3);
				__m512 fp8 = _mm512_add_ps(r1, d8);
				__m512 fn8 = _mm512_sub_ps(r1, d8);
				...
				r2 = _mm512_load_ps(outnext[w + 8]);
				r3 = _mm512_load_ps(outcur[w + 8]);
				r1 = _mm512_add_ps(r1, gridcorrection);
				r1 = _mm512_mul_ps(r1, _mm512_set1_ps(0.33333333333f));
...
void WienerFilter::ApplyWiener3D4_AVX512() noexcept
{
	// dft with 4 points
	...
	int w(0);
	const int outwidth8 = outwidth - outwidth % 8;
	const __m512 sigmaSquaredNoiseNormed8 = _mm512_set1_ps(sigmaSquaredNoiseNormed);
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (w = 0; w < outwidth8; w = w + 8)
			{
				...
				r3 = _mm512_sub_ps(r1, sigmaSquaredNoiseNormed8);
				r4 = _mm512_sub_ps(r2, sigmaSquaredNoiseNormed8);
...
void WienerFilter::ApplyWiener3D4_degrid_AVX512() noexcept
{
	// dft with 4 points
	...
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (w = 0; w < outwidth8; w = w + 8)
			{
				const __m512 gridcorrection = _mm512_mul_ps(_mm512_mul_ps(_mm512_load_ps(&gridsample[w][0]), gridfraction8), _mm512_set1_ps(4));
				__m512 r1 = _mm512_load_ps(&outprev2[w][0]);
				__m512 r2 = _mm512_load_ps(&outprev[w][0]);
				//outcur[w][0] - outnext[w][1] - outprev2[w][0] + outprev[w][1]
				//outcur[w][1] + outnext[w][0] - outprev2[w][1] - outprev[w][0]
				...
				r8 = _mm512_sub_ps(r8, r1); //-outprev2
				r5 = _mm512_maskz_add_ps(k1, r8, r6);
				r8 = _mm512_mask_sub_ps(r5, k2, r8, r6);

				r5 = _mm512_add_ps(r1, r2);
				r6 = _mm512_add_ps(r3, r4);
				r5 = _mm512_add_ps(r5, r6); // fcr1, fci1, fcr2, fci2
				r5 = _mm512_sub_ps(r5, gridcorrection);
				...
				r6 = _mm512_add_ps(r6, r8);
				r6 = _mm512_mul_ps(_mm512_add_ps(r6, gridcorrection), _mm512_set1_ps(0.25f));
...
void WienerFilter::ApplyWiener3D5_AVX512() noexcept
{
	// dft with 5 points
	// return result in outprev2
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (w = 0; w < outwidth8; w = w + 8) // 
			{
				...
				r1 = _mm512_sub_ps(psd, _mm512_set1_ps(sigmaSquaredNoiseNormed));
			...
			for (; w < outwidth; w++) // 
			{
				...
				float fcr = outprev2[w][0] + outprev[w][0] + outcur[w][0] + outnext[w][0] + outnext2[w][0]; // real cur
				float fci = outprev2[w][1] + outprev[w][1] + outcur[w][1] + outnext[w][1] + outnext2[w][1]; // im cur

				float psd = fp2r * fp2r + fp2i * fp2i + 1e-15f; // power spectrum density prev2
				float WienerFactor = max((psd - sigmaSquaredNoiseNormed) / psd, lowlimit); // limited Wiener filter
				fp2r *= WienerFactor; // apply filter on real  part	
				fp2i *= WienerFactor; // apply filter on imaginary part
...
void WienerFilter::ApplyWiener3D5_degrid_AVX512() noexcept
{
	// dft with 5 points
	// return result in outprev2
	constexpr float sin72 = 0.95105651629515357211643933337938f;// 2*pi/5
	...
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (w = 0; w < outwidth8; w = w + 8) // 
			{
				__m512 gridcorrection = _mm512_mul_ps(_mm512_load_ps(&gridsample[w][0]), gridfraction8);
				...
				r6 = _mm512_add_ps(r5, r6);
				__m512 fc = _mm512_add_ps(r6, r7);
				fc = _mm512_sub_ps(fc, gridcorrection);
			...
			for (; w < outwidth; w++) // 
			{
				const float gridcorrection0_5 = gridfraction * gridsample[w][0] * 5;
				const float gridcorrection1_5 = gridfraction * gridsample[w][1] * 5;
				float sum = (outprev2[w][0] + outnext2[w][0])*cos72 + (outprev[w][0] + outnext[w][0])*cos144 + +outcur[w][0];
				float dif = (-outprev2[w][1] + outnext2[w][1])*sin72 + (outprev[w][1] - outnext[w][1])*sin144;
				float fp2r = sum + dif; // real prev2
				float fn2r = sum - dif; // real next2
