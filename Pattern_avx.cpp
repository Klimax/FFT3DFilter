...
void PatternFilter::ApplyPattern2D_AVX() noexcept
{
...
void PatternFilter::ApplyPattern2D_degrid_AVX() noexcept
{
...
//-----------------------------------------------------------------------------------------
//
void PatternFilter::ApplyPattern3D2_AVX() noexcept
{
...
void PatternFilter::ApplyPattern3D2_degrid_AVX() noexcept
{
	// dft 3d (very short - 2 points)
...
void PatternFilter::ApplyPattern3D3_AVX() noexcept
{
...
void PatternFilter::ApplyPattern3D3_degrid_AVX() noexcept
{
	// dft 3d (very short - 3 points)
	constexpr float sin120 = 0.86602540378443864676372317075294f;//sqrtf(3.0f)*0.5f;
...
void PatternFilter::ApplyPattern3D4_AVX() noexcept
{
	// dft with 4 points
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 4)
			{
				...
				r6 = _mm256_mul_ps(r6, _mm256_set_ps(1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f));

				__m256 r7 = _mm256_addsub_ps(r3, r5); //outcur + outnext
				r7 = _mm256_sub_ps(r7, r1); //-outprev2
				r7 = _mm256_add_ps(r7, r6); // fpr1, fpi1, fpr2, fpi2 = +outprev

											//outcur[w][0] + outnext[w][1] - outprev2[w][0] - outprev[w][1]
											//outcur[w][1] - outnext[w][0] - outprev2[w][1] + outprev[w][0]
				r5 = _mm256_mul_ps(r5, _mm256_set_ps(-1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f));
				r6 = _mm256_mul_ps(r6, _mm256_set_ps(-1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f));

				__m256 r8 = _mm256_add_ps(r3, r5); //outcur + outnext
				r8 = _mm256_sub_ps(r8, r1); //-outprev2
				r8 = _mm256_add_ps(r8, r6); // fnr1, fni1, fnr2, fni2 = +outprev
											//
											//
				...
				r7 = _mm256_mul_ps(r7, _mm256_unpacklo_ps(r1, r1));
				r8 = _mm256_mul_ps(r8, _mm256_unpackhi_ps(r1, r1));
				r5 = _mm256_mul_ps(r5, _mm256_unpacklo_ps(r2, r2));
				r6 = _mm256_mul_ps(r6, _mm256_unpackhi_ps(r2, r2));
				r1 = _mm256_add_ps(r5, r6);
				r2 = _mm256_add_ps(r7, r8);
				r1 = _mm256_add_ps(r1, r2);
				r1 = _mm256_mul_ps(r1, _mm256_set1_ps(0.25f));

				r3 = _mm256_load_ps(&outcur[w + 4][0]);
				r4 = _mm256_load_ps(&outnext[w + 4][0]);
				_mm256_store_ps(&outprev2[w][0], r1);
...
void PatternFilter::ApplyPattern3D4_degrid_AVX() noexcept
{
	// dft with 4 points
	const __m256 lowlimit8 = _mm256_broadcast_ss(&lowlimit);
	...
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 4)
			{
				...
				r2 = _mm256_add_ps(r7, r8);
				r1 = _mm256_add_ps(r1, r2);
				r1 = _mm256_mul_ps(_mm256_add_ps(r1, gridcorrection), _mm256_set1_ps(0.25f));
...
void PatternFilter::ApplyPattern3D5_AVX() noexcept
{
	// dft with 5 points
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 4) // 
			{
				...
				r6 = _mm256_add_ps(r1, _mm256_mul_ps(r2, _mm256_set_ps(1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f))); //dif, sum, dif, sum
				r7 = _mm256_add_ps(_mm256_mul_ps(r3, _mm256_set_ps(1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f)), r4); //dif, sum, dif, sum
				...
				r6 = _mm256_add_ps(_mm256_mul_ps(r1, _mm256_set_ps(1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f)), r2); //dif, sum, dif, sum
				r7 = _mm256_add_ps(_mm256_mul_ps(r3, _mm256_set_ps(1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f)), r4); //dif, sum, dif, sum
				...
				fp2r = _mm256_shuffle_ps(fp2r, fp2r, _MM_SHUFFLE(3, 1, 2, 0));
				...
				fpr = _mm256_shuffle_ps(fpr, fpr, _MM_SHUFFLE(3, 1, 2, 0));
				psd = _mm256_add_ps(r1, r2);
				psd = _mm256_add_ps(psd, _mm256_set1_ps(1e-15f));
				r1 = _mm256_sub_ps(psd, _mm256_broadcast_ps((__m128*)&pattern3d[w]));
				r1 = _mm256_div_ps(r1, psd);
				...
				fnr = _mm256_shuffle_ps(fnr, fnr, _MM_SHUFFLE(3, 1, 2, 0));
				psd = _mm256_add_ps(r1, r2);
				psd = _mm256_add_ps(psd, _mm256_set1_ps(1e-15f));
				r1 = _mm256_sub_ps(psd, _mm256_broadcast_ps((__m128*)&pattern3d[w]));
				r1 = _mm256_div_ps(r1, psd);
				...
				fn2r = _mm256_shuffle_ps(fn2r, fn2r, _MM_SHUFFLE(3, 1, 2, 0));
				psd = _mm256_add_ps(r1, r2);
				psd = _mm256_add_ps(psd, _mm256_set1_ps(1e-15f));
				r1 = _mm256_sub_ps(psd, _mm256_broadcast_ps((__m128*)&pattern3d[w]));
				r1 = _mm256_div_ps(r1, psd);
...
void PatternFilter::ApplyPattern3D5_degrid_AVX() noexcept
{
	// dft with 5 points
	// return result in outprev2
	const __m256 sincos72 = _mm256_set_ps(0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938,
		...
		for (int bx = 2; bx < nox - 2; bx++)
		{
			...
			for (int h = 0; h < bh; h++)
			{
				for (int w = 0; w < outwidth; w = w + 4)
				{
					...
					__m128 r2s = _mm256_extractf128_ps(grid, 1);
					r2s = _mm_shuffle_ps(r2s, r2s, _MM_SHUFFLE(0, 0, 0, 2));
					r2s = _mm_add_ps(r2s, _mm256_castps256_ps128(grid));
					r2s = _mm_add_ps(r2s, _mm_broadcast_ss(&sigmaSquaredcur));
					_mm_store_ss(&sigmaSquaredcur, r2s);
				}
				outcur += outpitch;
				pwin += outpitch;
				gridsample += outpitch;
