...
//-----------------------------------------------------------------------------------------
//
void WienerFilter::ApplyWiener2D_SSE2() noexcept
{
		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 2) // not skip first
				{
					...
					const __m128 sharp = _mm_loadl_pi(_mm_setzero_ps(), (__m64*) &wsharpen[w]);
					const __m128 r7 = _mm_mul_ps(_mm_load1_ps(&sharpen), _mm_unpacklo_ps(sharp, sharp));
...
void WienerFilter::ApplyWiener2D_degrid_SSE2() noexcept
{
	if (sharpen == 0 && dehalo == 0)// no sharpen, no dehalo
	{
		for (int block = start_block; block < blocks; block++)
...
void WienerFilter::ApplyWiener3D2_SSE2() noexcept
{
...
void WienerFilter::ApplyWiener3D2_degrid_SSE2() noexcept
{
	// dft 3d (very short - 2 points)
	for (int block = start_block; block < blocks; block++)
	{
...
void WienerFilter::ApplyWiener3D3_SSE2() noexcept
{
...
void WienerFilter::ApplyWiener3D3_degrid_SSE2() noexcept
{
	// dft 3d (very short - 3 points)
	constexpr float sin120 = 0.86602540378443864676372317075294f;//sqrtf(3.0f)*0.5f;
...
void WienerFilter::ApplyWiener3D4_SSE2() noexcept
{
	// dft with 4 points
	for (int block = start_block; block < blocks; block++)
	{
...
void WienerFilter::ApplyWiener3D4_degrid_SSE2() noexcept
{
	// dft with 4 points
	...
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 2)
			{
				...
				//t3: fcr1, fcr2, fp2r1, fp2r2
				//t4: fci1, fci2, fp2i1, fp2i2
				const __m128 sh1 = _mm_shuffle_ps(r1, r2, _MM_SHUFFLE(0, 2, 0, 2));
				const __m128 sh2 = _mm_shuffle_ps(r1, r2, _MM_SHUFFLE(1, 3, 1, 3));
				const __m128 sh3 = _mm_shuffle_ps(r3, r4, _MM_SHUFFLE(0, 2, 0, 2));
				const __m128 sh4 = _mm_shuffle_ps(r3, r4, _MM_SHUFFLE(1, 3, 1, 3));
				...
				r7 = _mm_mul_ps(r7, _mm_shuffle_ps(r1, r1, _MM_SHUFFLE(0, 0, 1, 1)));
				//r4 = ;
				r8 = _mm_mul_ps(r8, _mm_shuffle_ps(r1, r1, _MM_SHUFFLE(2, 2, 3, 3)));

				//r3 = ;
				r5 = _mm_mul_ps(r5, _mm_shuffle_ps(r2, r2, _MM_SHUFFLE(0, 0, 1, 1)));
				//r4 = ;
				r6 = _mm_mul_ps(r6, _mm_shuffle_ps(r2, r2, _MM_SHUFFLE(2, 2, 3, 3)));
...
void WienerFilter::ApplyWiener3D5_SSE2() noexcept
{
	// dft with 5 points
...
void WienerFilter::ApplyWiener3D5_degrid_SSE2() noexcept
{
	// dft with 5 points
	const __m128 sincos72 = _mm_set_ps(0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938);
	const __m128 cossin72 = _mm_set_ps(0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f);
