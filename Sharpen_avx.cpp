		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 4)
				{
					...
					const __m256 psd4 = _mm256_hadd_ps(r1, r1); //psd1, psd2, psd1, psd2
		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 4)
				{
					const __m256 cur = _mm256_load_ps(outcur[w]);
					const __m256 r1 = _mm256_mul_ps(cur, cur);
					const __m256 psd4 = _mm256_hadd_ps(r1, r1); //psd1, -, psd2, -, psd3, -, psd4, -
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 4)
				{
					...
					const __m256 wdehalo4 = _mm256_broadcast_ps((__m128*)&wdehalo[w]);
					r3 = _mm256_add_ps(psd4, ht2n8);
					r4 = _mm256_mul_ps(psd4, dehalo8);
