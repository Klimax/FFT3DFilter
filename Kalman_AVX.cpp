...
void KalmanFilter::ApplyKalman_AVX() noexcept
{
...
void KalmanFilter::ApplyKalmanPattern_AVX() noexcept
{
	// return result in outLast
	int w(0);
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // 
		{
			for (w = 0; w < outwidth4; w = w + 4)
			{
				...
				__m256 r4 = _mm256_mul_ps(gain, cur);
				r3 = _mm256_mul_ps(gain, last);
