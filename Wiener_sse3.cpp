...
//-----------------------------------------------------------------------------------------
//
void WienerFilter::ApplyWiener2D_SSE3() noexcept
{
		...
		for (int block = start_block; block < blocks; block++)
		{
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 2) // not skip first
				{
					...
					__m128 r7 = _mm_mul_ps(r3, _mm_loadl_pi(_mm_setzero_ps(), (__m64*) &wdehalo[w]));
...
void WienerFilter::ApplyWiener2D_degrid_SSE3() noexcept
{
	if (sharpen == 0 && dehalo == 0)// no sharpen, no dehalo
	{
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 2) // not skip first v.1.2
				{
					...
					r3 = _mm_unpacklo_ps(r3, r3);
					r1 = _mm_mul_ps(r4, r3);//corrected * wienerfactor
...
void WienerFilter::ApplyWiener3D2_SSE3() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++)
		{
			...
			for (int w = 0; w < outwidth; w = w + 2)
			{
				...
				__m128 psd4 = _mm_hadd_ps(r1, r2);

				psd4 = _mm_add_ps(psd4, _mm_set1_ps(1e-15f));
...
void WienerFilter::ApplyWiener3D2_degrid_SSE3() noexcept
{
	// dft 3d (very short - 2 points)
	for (int block = start_block; block < blocks; block++)
	{
...
void WienerFilter::ApplyWiener3D3_SSE3() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 2) // 
			{
				...
				fc4 = _mm_mul_ps(_mm_shuffle_ps(r1, r1, _MM_SHUFFLE(1, 1, 0, 0)), fc4);
				fp4 = _mm_mul_ps(_mm_shuffle_ps(r1, r1, _MM_SHUFFLE(3, 3, 2, 2)), fp4);
				fn4 = _mm_mul_ps(_mm_shuffle_ps(r3, r3, _MM_SHUFFLE(1, 1, 0, 0)), fn4);
...
void WienerFilter::ApplyWiener3D3_degrid_SSE3() noexcept
{
	// dft 3d (very short - 3 points)
	constexpr float sin120 = 0.86602540378443864676372317075294f;//sqrtf(3.0f)*0.5f;
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 2) // 
			{
				...
				r2 = _mm_shuffle_ps(r1, r1, _MM_SHUFFLE(2, 2, 1, 1));
...
void WienerFilter::ApplyWiener3D4_SSE3() noexcept
{
	// dft with 4 points
	for (int block = start_block; block < blocks; block++)
	{
...
void WienerFilter::ApplyWiener3D4_degrid_SSE3() noexcept
{
	// dft with 4 points
...
void WienerFilter::ApplyWiener3D5_SSE3() noexcept
{
	// dft with 5 points
...
void WienerFilter::ApplyWiener3D5_degrid_SSE3() noexcept
{
	// dft with 5 points
	// return result in outprev2
	const __m128 sincos72 = _mm_set_ps(0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938);
