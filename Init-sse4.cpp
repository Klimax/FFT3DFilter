...
void FFT3DFilter::InitOverlapPlane_SSE4(float *__restrict inp0, const BYTE *__restrict srcp0) noexcept
{
	int in_t(0), w(0);
...
void FFT3DFilter::InitOverlapPlane_wt2_SSE4(float *__restrict inp0, const BYTE *__restrict srcp0) noexcept
{
	int w(0), in_t(0);
	const BYTE *__restrict srcp = srcp0;// + (hrest/2)*coverpitch + wrest/2; // centered
	const int xoffset = bh * bw - (bw - ow); // skip frames
