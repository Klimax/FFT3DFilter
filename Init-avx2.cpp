...
void FFT3DFilter::InitOverlapPlane_AVX2(float *__restrict inp0, const BYTE *__restrict srcp0) noexcept
{
	int in_t(0), w(0);
	...
	// first top (big non-overlapped) part
	{
		for (int h = 0; h < oh; h++)
		{
			...
			const __m256 wanyl8 = _mm256_set1_ps(wanyl_);
			for (w = 0; w < ow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				__m256i r1i = _mm256_cvtepu8_epi32(_mm_loadl_epi64((__m128i*)&srcp[w]));
				r1i = _mm256_sub_epi32(r1i, planebase8);
				__m256 r1 = _mm256_cvtepi32_ps(r1i);
				r1 = _mm256_mul_ps(r1, wanyl8);
				r1 = _mm256_mul_ps(r1, _mm256_load_ps(&wanxl[w]));
				_mm256_store_ps(&inp[w], r1);
			}
			...
			for (; w < dbwow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				__m256i r1i = _mm256_cvtepu8_epi32(_mm_loadl_epi64((__m128i*)&srcp[w]));
				r1i = _mm256_sub_epi32(r1i, planebase8);
				__m256 r1 = _mm256_cvtepi32_ps(r1i);
			...
			for (int ihx = 1; ihx < nox; ihx++) // middle horizontal blocks
			{
				for (w = 0; w < ow8; w = w + 8)   // left part  (non-overlapped) row of first block
				{
					...
					__m256 r1 = _mm256_cvtepi32_ps(r1i);
					r1 = _mm256_mul_ps(r1, wanyl8);
					_mm256_store_ps(&inp[w], _mm256_mul_ps(r1, _mm256_load_ps(&wanxr[w])));
					_mm256_store_ps(&inp[w + xoffset], _mm256_mul_ps(r1, _mm256_load_ps(&wanxl[w])));
				}
				...
				for (w = 0; w < d2bwow8; w = w + 8)   // left part  (non-overlapped) row of first block
				{
					__m256i r1i = _mm256_cvtepu8_epi32(_mm_loadl_epi64((__m128i*)&srcp[w]));
					r1i = _mm256_sub_epi32(r1i, planebase8);
					const __m256 r1 = _mm256_cvtepi32_ps(r1i);
					_mm256_store_ps(&inp[w], _mm256_mul_ps(r1, wanyl8));
				}
			...
			for (w = 0; w < ow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				...
				__m256 r1 = _mm256_cvtepi32_ps(r1i);
				r1 = _mm256_mul_ps(r1, wanyl8);
				r1 = _mm256_mul_ps(r1, _mm256_load_ps(&wanxr[w]));
				_mm256_store_ps(&inp[w], r1);
			}
		...
		for (int h = oh; h < bh - oh; h++)
		{
			...
			for (; w < dbwow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				__m256i r1i = _mm256_cvtepu8_epi32(_mm_loadl_epi64((__m128i*)&srcp[w]));
				r1i = _mm256_sub_epi32(r1i, planebase8);
				_mm256_store_ps(&inp[w], _mm256_cvtepi32_ps(r1i));
			}
			for (; w < dbwow; w++)   // left part  (non-overlapped) row of first block
			{
	...
	for (int ihy = 1; ihy < noy; ihy++) // middle vertical
	{
		for (int h = 0; h < oh; h++) // top overlapped part
		{
			...
			const __m256 wanyr8 = _mm256_set1_ps(wanyr_);
			...
			for (w = 0; w < ow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				...
				__m256 r1 = _mm256_cvtepi32_ps(r1i);
				r1 = _mm256_mul_ps(r1, _mm256_load_ps(&wanxl[w]));
				_mm256_store_ps(&inp[w], _mm256_mul_ps(r1, wanyr8));
				_mm256_store_ps(&inp[w + yoffset], _mm256_mul_ps(r1, wanyl8));
			}
...
void FFT3DFilter::InitOverlapPlane_wt2_AVX2(float *__restrict inp0, const BYTE *__restrict srcp0) noexcept
{
	int w(0), in_t(0);
	const BYTE *__restrict srcp = srcp0;// + (hrest/2)*coverpitch + wrest/2; // centered
	const int xoffset = bh * bw - (bw - ow); // skip frames
	...
	// first top (big non-overlapped) part
	{
		for (int h = 0; h < oh; h++)
		{
			...
			for (w = ow; w < dbwow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				__m256i r1i = _mm256_cvtepu8_epi32(_mm_loadl_epi64((__m128i*)&srcp[w]));
				r1i = _mm256_sub_epi32(r1i, planebase8);
				_mm256_store_ps(&inp[w], _mm256_cvtepi32_ps(r1i));
			...
			for (int ihx = 1; ihx < nox; ihx++) // middle horizontal blocks
			{
				for (w = 0; w < ow8; w = w + 8)   // left part  (non-overlapped) row of first block
				{
					...
					r1i = _mm256_sub_epi32(r1i, planebase8);
					_mm256_store_ps(&inp[w], _mm256_cvtepi32_ps(r1i));
					_mm256_store_ps(&inp[w + xoffset], _mm256_cvtepi32_ps(r1i));
				}
				for (; w < ow; w++)   // first part (overlapped) row of block
				{
					const float ftmp = float(srcp[w] - planeBase);   // Copy each byte from source to float array
	...
	for (int ihy = 1; ihy < noy; ihy++) // middle vertical
	{
		...
		for (int h = 0; h < oh; h++) // top overlapped part
		{
			...
			for (w = 0; w < ow8; w = w + 8)   // left part  (non-overlapped) row of first block
			{
				...
				r1i = _mm256_sub_epi32(r1i, planebase8);
				_mm256_store_ps(&inp[w], _mm256_cvtepi32_ps(r1i));
				_mm256_store_ps(&inp[w + yoffset], _mm256_cvtepi32_ps(r1i));
			}
			for (; w < ow; w++)   // first half line of first block
			{
				const float ftmp = float(srcp[w] - planeBase);
			...
			for (int ihx = 1; ihx < nox; ihx++) // middle blocks
			{
				...
				for (w = 0; w < d2bwow8; w = w + 8)   // left part  (non-overlapped) row of first block
				{
					...
					const __m256 r1 = _mm256_cvtepi32_ps(r1i);
					_mm256_store_ps(&inp[w], r1);
					_mm256_store_ps(&inp[w + yoffset], r1);
				}
				for (; w < d2bwow; w++)   // half non-overlapped line of block
				{
					const float ftmp = float(srcp[w] - planeBase);   // Copy each byte from source to float array
