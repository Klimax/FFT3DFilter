...
//  SSE4 version of filtering functions
//
...
void KalmanFilter::ApplyKalman_SSE4() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // 
		{
			...
			for (w = 0; w < outwidth2; w = w + 2)
			{
				...
				_mm_store_ps(covarProcess[w], _mm_blendv_ps(r3, covarNoiseNormed4, mask));
...
void KalmanFilter::ApplyKalmanPattern_SSE4() noexcept
{
	// return result in outLast
	int w(0);
	fftwf_complex *__restrict covar = covar_in, *__restrict covarProcess = covarProcess_in;
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // 
		{
			for (w = 0; w < outwidth2; w = w + 2)
			{
				...
				_mm_store_ps(covarProcess[w], _mm_blendv_ps(r3, cnn4, mask));
