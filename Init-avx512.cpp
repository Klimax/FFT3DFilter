...
void FFT3DFilter::InitOverlapPlane_AVX512(float *__restrict inp0, const BYTE *__restrict srcp0) noexcept
{
	int in_t(0), w(0);
	...
	// first top (big non-overlapped) part
	{
		for (int h = 0; h < oh; h++)
		{
			...
			const __m512 wanyl16 = _mm512_set1_ps(wanyl_);
			for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				__m512i r1i = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i*)&srcp[w]));
				r1i = _mm512_sub_epi32(r1i, planebase16);
				__m512 r1 = _mm512_cvtepi32_ps(r1i);
				r1 = _mm512_mul_ps(r1, wanyl16);
				r1 = _mm512_mul_ps(r1, _mm512_load_ps(&wanxl[w]));
				_mm512_store_ps(&inp[w], r1);
			}
			for (; w < ow; w++)   // left part  (non-overlapped) row of first block
			{
				inp[w] = float(wanxl[w] * wanyl_ * (srcp[w] - planeBase));   // Copy each byte from source to float array
			}

			for (; w < dbwow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				__m512i r1i = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i*)&srcp[w]));
				r1i = _mm512_sub_epi32(r1i, planebase16);
				__m512 r1 = _mm512_cvtepi32_ps(r1i);
			...
			{
				inp[w] = float(wanyl_ * (srcp[w] - planeBase));   // Copy each byte from source to float array
			}
			inp += bw - ow;
			srcp += bw - ow;
			for (int ihx = 1; ihx < nox; ihx++) // middle horizontal blocks
			{
				for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
				{
					...
					__m512 r1 = _mm512_cvtepi32_ps(r1i);
					r1 = _mm512_mul_ps(r1, wanyl16);
					_mm512_store_ps(&inp[w], _mm512_mul_ps(r1, _mm512_load_ps(&wanxr[w])));
					_mm512_store_ps(&inp[w + xoffset], _mm512_mul_ps(r1, _mm512_load_ps(&wanxl[w])));
				}
				for (; w < ow; w++)   // first part (overlapped) row of block
				{
					const float ftmp = float(wanyl_ * (srcp[w] - planeBase));   // Copy each byte from source to float array
				...
				for (w = 0; w < d2bwow16; w = w + 16)   // left part  (non-overlapped) row of first block
				{
					__m512i r1i = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i*)&srcp[w]));
					r1i = _mm512_sub_epi32(r1i, planebase16);
					__m512 r1 = _mm512_cvtepi32_ps(r1i);
			...
			for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				...
				__m512 r1 = _mm512_cvtepi32_ps(r1i);
				r1 = _mm512_mul_ps(r1, wanyl16);
				r1 = _mm512_mul_ps(r1, _mm512_load_ps(&wanxr[w]));
				_mm512_store_ps(&inp[w], r1);
			}
			for (; w < ow; w++)   // last part (non-overlapped) of line of last block
			{
				inp[w] = float(wanxr[w] * wanyl_ * (srcp[w] - planeBase));   // Copy each byte from source to float array
			}
			inp += ow;
			srcp += ow;
		...
		for (int h = oh; h < bh - oh; h++)
		{
			...
			for (; w < dbwow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				__m512i r1i = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i*)&srcp[w]));
				r1i = _mm512_sub_epi32(r1i, planebase16);
				_mm512_store_ps(&inp[w], _mm512_cvtepi32_ps(r1i));
			}
			for (; w < dbwow; w++)   // left part  (non-overlapped) row of first block
			{
			...
			for (int ihx = 1; ihx < nox; ihx++) // middle horizontal blocks
			{
				for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
				{
					__m512i r1i = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i*)&srcp[w]));
					r1i = _mm512_sub_epi32(r1i, planebase16);
					const __m512 r1 = _mm512_cvtepi32_ps(r1i);
					_mm512_store_ps(&inp[w], _mm512_mul_ps(r1, _mm512_load_ps(&wanxr[w])));
					_mm512_store_ps(&inp[w + xoffset], _mm512_mul_ps(r1, _mm512_load_ps(&wanxl[w])));
				}
				for (; w < ow; w++)   // first part (overlapped) row of block
	...
	for (int ihy = 1; ihy < noy; ihy++) // middle vertical
	{
		for (int h = 0; h < oh; h++) // top overlapped part
		{
			...
			const __m512 wanyr16 = _mm512_set1_ps(wanyr_);
			...
			for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				...
				__m512 r1 = _mm512_cvtepi32_ps(r1i);
				r1 = _mm512_mul_ps(r1, _mm512_load_ps(&wanxl[w]));
				_mm512_store_ps(&inp[w], _mm512_mul_ps(r1, wanyr16));
				_mm512_store_ps(&inp[w + yoffset], _mm512_mul_ps(r1, wanyl16));
			}
...
void FFT3DFilter::InitOverlapPlane_wt2_AVX512(float *__restrict inp0, const BYTE *__restrict srcp0) noexcept
{
	int w(0), in_t(0);
	const BYTE *__restrict srcp = srcp0;// + (hrest/2)*coverpitch + wrest/2; // centered
	const int xoffset = bh * bw - (bw - ow); // skip frames
	...
	const int ow16 = ow - ow % 16;
	const int ow4 = ow - ow % 4;
	const int outsize16 = outsize - outsize % 16;
	...
	// first top (big non-overlapped) part
	{
		for (int h = 0; h < oh; h++)
		{
			...
			for (w = ow; w < dbwow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				__m512i r1i = _mm512_cvtepu8_epi32(_mm_load_si128((__m128i*)&srcp[w]));
				r1i = _mm512_sub_epi32(r1i, planebase16);
				_mm512_store_ps(&inp[w], _mm512_cvtepi32_ps(r1i));
			...
			for (int ihx = 1; ihx < nox; ihx++) // middle horizontal blocks
			{
				for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
				{
					...
					r1i = _mm512_sub_epi32(r1i, planebase16);
					_mm512_store_ps(&inp[w], _mm512_cvtepi32_ps(r1i));
					_mm512_store_ps(&inp[w + xoffset], _mm512_cvtepi32_ps(r1i));
				}
				for (; w < ow; w++)   // first part (overlapped) row of block
				{
					const float ftmp = float(srcp[w] - planeBase);   // Copy each byte from source to float array
	...
	for (int ihy = 1; ihy < noy; ihy++) // middle vertical
	{
		...
		for (int h = 0; h < oh; h++) // top overlapped part
		{
			...
			for (w = 0; w < ow16; w = w + 16)   // left part  (non-overlapped) row of first block
			{
				...
				r1i = _mm512_sub_epi32(r1i, planebase16);
				_mm512_store_ps(&inp[w], _mm512_cvtepi32_ps(r1i));
				_mm512_store_ps(&inp[w + yoffset], _mm512_cvtepi32_ps(r1i));
			}
			for (; w < ow; w++)   // first half line of first block
			{
				const float ftmp = float(srcp[w] - planeBase);
			...
			for (int ihx = 1; ihx < nox; ihx++) // middle blocks
			{
				...
				for (w = 0; w < d2bwow16; w = w + 16)   // left part  (non-overlapped) row of first block
				{
					...
					const __m512 r1 = _mm512_cvtepi32_ps(r1i);
					_mm512_store_ps(&inp[w], r1);
					_mm512_store_ps(&inp[w + yoffset], r1);
				}
				for (; w < d2bwow; w++)   // half non-overlapped line of block
				{
					const float ftmp = float(srcp[w] - planeBase);   // Copy each byte from source to float array
