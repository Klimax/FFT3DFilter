...
//  SSE3 version of filtering functions
//
...
void PatternFilter::ApplyPattern2D_SSE3() noexcept
{
	for (int block = start_block; block < blocks; block++)
	{
		...
		for (int h = 0; h < bh; h++) // middle
		{
			for (int w = 0; w < outwidth; w = w + 2)
			{
				...
				const __m128 r2 = _mm_sub_ps(r1, pf);
				r1 = _mm_div_ps(r2, r1);
				r1 = _mm_max_ps(r1, _mm_load1_ps(&lowlimit));
				r1 = _mm_shuffle_ps(r1, r1, _MM_SHUFFLE(1, 1, 0, 0));
...
void PatternFilter::ApplyPattern2D_degrid_SSE3() noexcept
{
...
//-----------------------------------------------------------------------------------------
//
void PatternFilter::ApplyPattern3D2_SSE3() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++)
		{
			...
			for (int w = 0; w < outwidth; w = w + 2)
			{
				...
				__m128 psds4 = _mm_hadd_ps(r1, r1);
				__m128 psdd4 = _mm_hadd_ps(r2, r2);

				psds4 = _mm_add_ps(psds4, _mm_set1_ps(1e-15f));
				psdd4 = _mm_add_ps(psdd4, _mm_set1_ps(1e-15f));

				const __m128 pf = _mm_loadl_pi(_mm_setzero_ps(), (__m64*) &pattern3d[w]);
				...
				r1 = _mm_shuffle_ps(WienerFactors4, WienerFactors4, _MM_SHUFFLE(1, 1, 0, 0));
				r2 = _mm_shuffle_ps(WienerFactord4, WienerFactord4, _MM_SHUFFLE(1, 1, 0, 0));
...
void PatternFilter::ApplyPattern3D2_degrid_SSE3() noexcept
{
	// dft 3d (very short - 2 points)
...
void PatternFilter::ApplyPattern3D3_SSE3() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 2) // 
			{
				...
				pf = _mm_shuffle_ps(pf, pf, _MM_SHUFFLE(1, 0, 1, 0));
				...
				r2 = _mm_shuffle_ps(r1, r1, _MM_SHUFFLE(3, 3, 2, 2));
				r1 = _mm_shuffle_ps(r1, r1, _MM_SHUFFLE(1, 1, 0, 0));
				r3 = _mm_shuffle_ps(r3, r3, _MM_SHUFFLE(1, 1, 0, 0));
...
void PatternFilter::ApplyPattern3D3_degrid_SSE3() noexcept
{
	// dft 3d (very short - 3 points)
	constexpr float sin120 = 0.86602540378443864676372317075294f;//sqrtf(3.0f)*0.5f;
...
void PatternFilter::ApplyPattern3D4_SSE3() noexcept
{
	// dft with 4 points
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			...
			for (int w = 0; w < outwidth; w = w + 2)
			{
				...
				__m128 r7 = _mm_addsub_ps(r3, r5); //outcur + outnext
...
void PatternFilter::ApplyPattern3D4_degrid_SSE3() noexcept
{
	// dft with 4 points
	for (int block = start_block; block < blocks; block++)
	{
...
void PatternFilter::ApplyPattern3D5_SSE3() noexcept
{
	// dft with 5 points
...
void PatternFilter::ApplyPattern3D5_degrid_SSE3() noexcept
{
	// dft with 5 points
	// return result in outprev2
	const __m128 sincos72 = _mm_set_ps(0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938);
