...
void WienerFilter::ApplyWiener2D_AVX() noexcept
{
...
void WienerFilter::ApplyWiener2D_degrid_AVX() noexcept
{
	if (sharpen == 0 && dehalo == 0)// no sharpen, no dehalo
	{
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 4) // not skip first v.1.2
				{
					...
					r3 = _mm256_unpacklo_ps(r3, r3);
					r1 = _mm256_mul_ps(r4, r3);//corrected
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 4) // not skip first
				{
					...
					r1 = _mm256_mul_ps(r4, wienerfactor4);//corrected
					r1 = _mm256_add_ps(gridcorrection, r1);// final
					_mm256_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
		...
		for (int block = start_block; block < blocks; block++)
		{
			...
			for (int h = 0; h < bh; h++) // middle
			{
				for (int w = 0; w < outwidth; w = w + 4) // not skip first
				{
					...
					r1 = _mm256_mul_ps(r4, wienerfactor8);//corrected
					r1 = _mm256_add_ps(gridcorrection, r1);// final
					_mm256_store_ps(outcur[w], r1);
				}
				_mm_prefetch((const char*)(outcur + outpitch), _MM_HINT_T0);
...
void WienerFilter::ApplyWiener3D2_AVX() noexcept
{
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++)
		{
			...
			for (int w = 0; w < outwidth; w = w + 4)
			{
				...
				__m256 psd8 = _mm256_hadd_ps(r1, r1);
...
void WienerFilter::ApplyWiener3D2_degrid_AVX() noexcept
{
	// dft 3d (very short - 2 points)
	for (int block = start_block; block < blocks; block++)
	{
...
void WienerFilter::ApplyWiener3D3_AVX() noexcept
{
...
void WienerFilter::ApplyWiener3D3_degrid_AVX() noexcept
{
	// dft 3d (very short - 3 points)
	constexpr float sin120 = 0.86602540378443864676372317075294f;//sqrtf(3.0f)*0.5f;
...
void WienerFilter::ApplyWiener3D4_AVX() noexcept
{
	// dft with 4 points
...
void WienerFilter::ApplyWiener3D4_degrid_AVX() noexcept
{
	// dft with 4 points
...
void WienerFilter::ApplyWiener3D5_AVX() noexcept
{
	// dft with 5 points
	...
	for (int block = start_block; block < blocks; block++)
	{
		for (int h = 0; h < bh; h++) // first half
		{
			for (int w = 0; w < outwidth; w = w + 4) // 
			{
				...
				const __m256 r3 = _mm256_load_ps(outprev[w]);
				const __m256 r4 = _mm256_load_ps(outnext[w]);
...
void WienerFilter::ApplyWiener3D5_degrid_AVX() noexcept
{
	// dft with 5 points
	const __m256 sincos72 = _mm256_set_ps(0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938,
		0.30901699437494742410229341718282f, 0.95105651629515357211643933337938, 0.30901699437494742410229341718282f, 0.95105651629515357211643933337938);
